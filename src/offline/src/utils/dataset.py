...
class Dataset:
    ...
    def __init__(
        ...
        self.user_ids = np.array(self.df_users['user_id'].drop_duplicates())
        self.movie_ids = np.array(self.df_movies['movie_id'].drop_duplicates())
    ...
    def to_rating_matrix(self, rating_matrix_csv: str) -> None:
        ...
        # 評価値行列をCSVファイルに出力する。
        df_rating_matrix.to_csv(self.local_path / rating_matrix_csv, index=True, header=True, sep='\t')
    ...
    def read_rating_matrix(self, rating_matrix_csv: str) -> None:
        ...
        # 評価値行列を読み込む。
        self.df_rating_matrix = pd.read_csv(self.local_path / rating_matrix_csv, index_col=0, header=0, sep='\t')
    ...
    def to_movies_movies_similarity_matrix(self, movies_movies_similarity_matrix_csv: str) -> None:
        ...
        df_movies_movies_similarity_matrix.to_csv(self.local_path / movies_movies_similarity_matrix_csv, index=True, header=True, sep='\t')
    ...
    def read_movies_movies_similarity_matrix(self, movies_movies_similarity_matrix_csv: str) -> None:
        ...
        # 映画-映画類似度行列を読み込む。
        self.df_movies_movies_similarity_matrix = pd.read_csv(self.local_path / movies_movies_similarity_matrix_csv, index_col=0, header=0, sep='\t')
    ...
    def to_atomic(self, dataset_name: str) -> None:
        ...
        def get_genres(movie_id: int) -> str:
            ...
            genre_names = [self.df_genres.query('genre_id==@genre_id')['genre_name'].values[0] for genre_id in genre_ids]
        ...
        for movie in self.df_movies.itertuples():
            genres = get_genres(movie.movie_id)
            rows.append([movie.movie_id, movie.title, movie.year, movie.imdb_id, movie.tmdb_id, genres])
        df_item = pd.DataFrame(rows, columns=['movie_id:token', 'title:token_seq', 'year:token', 'imbd_id:token', 'tmdb_id:token', 'genres:token_seq'])

        # 各データを出力する。
        df_inter.to_csv(self.local_path / (dataset_name + '.inter'), index=False, header=True, sep='\t')
        df_user.to_csv(self.local_path / (dataset_name + '.user'), index=False, header=True, sep='\t')
        df_item.to_csv(self.local_path / (dataset_name + '.item'), index=False, header=True, sep='\t')
