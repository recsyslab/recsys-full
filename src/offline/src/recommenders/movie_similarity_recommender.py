...
class MovieSimilarityRecommender(BaseRecommender):
    """映画類似度ベース推薦システム
    """

    def recommend(self, dataset: Dataset, top_k: int, **kwargs) -> RecommendResult:
        """映画類似度ベース推薦システムによる推薦結果を返す。
        ...
        rows = []
        for i in tqdm(range(len(S)), desc='processing MovieSimilarityRecommender.recommend'):
