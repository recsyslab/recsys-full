...
# cannot unpickle RecBole's checkpoint (it stores a Config object, not just
# tensors). The checkpoint here is one this script just trained and saved
# locally, so it's trusted.
...
class BPRRecommender(BaseRecommender):
    ...
    def recommend(self, dataset: Dataset, top_k: int, **kwargs) -> RecommendResult:
        ...
        config['data_path'] = dataset.local_path
